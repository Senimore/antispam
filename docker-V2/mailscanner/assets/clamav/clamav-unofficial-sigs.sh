...
si_url="www.securiteinfo.com/get/signatures/2ba3c11f37dd3a8d74db71b5fae9afc1ef9a40bf27cf9189961e73b4250cd5b115aa3702bc56bc65a70bd40105bc441f23bd65bc88c59b06dffed6ff6d765803"
      ...
      then
         ...
         for db_file in $si_dbs ; do
            ...
            if curl $curl_proxy $curl_output_level --connect-timeout "$curl_connect_timeout" \
               --max-time "$curl_max_time" -L -R $z_opt --insecure -o $si_dir/$db_file https://$si_url/$db_file
               then
                  loop="1"
      ...
      then
         ...
         for db_file in $mbl_dbs ; do
            ...
            if curl $curl_proxy $curl_output_level -R --connect-timeout "$curl_connect_timeout" \
               --max-time "$curl_max_time" --insecure -o $mbl_dir/$db_file https://lists.malwarepatrol.net/cgi/getfile?receipt=f1452107560&product=8&list=clamav_basic
               then
